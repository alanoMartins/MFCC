...
class MFCC:
    ...
    def execute(self, signal, rate):
        ...
        banks = self.filter_banks(signal, rate)
        mfcc = self.mfcc(banks)

        banks = self.normilize(banks)
        mfcc = self.normilize(mfcc)
    ...
    def filter_banks(self, signal, rate):
        ...
        low_freq_mel = 0
        high_freq_mel = (2595 * numpy.log10(1 + (rate / 2) / 700))  # Convert Hz to Mel
        ...
        return 20 * numpy.log10(filter_banks)  # dB
    ...
    def normilize(self, arr):
        arr -= (numpy.mean(arr, axis=0) + 1e-8)
        return arr